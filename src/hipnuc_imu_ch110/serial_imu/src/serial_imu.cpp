...
#define IMU_SERIAL   "/dev/USBport-imuCH110"
#define BAUD         (115200)
#define GRA_ACC      (9.8)
	...
	while(ros::ok())
	{
		...
		if(num!=0)
		{
			...
			if(num > 0)
			{
				...
				imu_data.header.frame_id = "imu_ch110_link";

				imu_0x91_msg.header.stamp = ros::Time::now(); 
				imu_0x91_msg.header.frame_id = "imu_ch110_0x91_link";

				imu_0x62_msg.header.stamp = ros::Time::now();
				imu_0x62_msg.header.frame_id = "imu_ch110_0x62_link";
