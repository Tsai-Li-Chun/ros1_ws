...
class localization_error_calculation(object):
    ...
    def point_err_calculation(self, p):
        #calculate distance
        dist = math.sqrt((self.odom_pose.pose.position.x - self.model_pose.pose.position.x)**2 + (self.odom_pose.pose.position.y - self.model_pose.pose.position.y)**2)
        dist = round(dist,2)
        
        #caluculate angular error
        odom_yaw = self.quaternion_to_euler(self.odom_pose.pose.orientation)
